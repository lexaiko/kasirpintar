            ...
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                    <li class="inline-flex items-center">
                        <a href="/admin/transaksi" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600 dark:text-gray-400 dark:hover:text-white">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="/admin/transaksi" class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">Edit transaksi</a>
                        </div>
                    </li>
                </ol>
            </nav>
                ...
                <div class="grid gap-2 sm:grid-cols-2 sm:gap-2">
                        ...
                        <select id="id_pelanggan" name="id_pelanggan"
                            ...
                            @foreach ($pelanggan as $k)
                                <option value="{{ $k->id_pelanggan }}"
                                    {{ old('id_pelanggan', $transaksi->id_pelanggan) == $k->id_pelanggan ? 'selected' : '' }}>{{ $k->nama }}
                                </option>
                            @endforeach
                        </select>
                        @error('id_pelanggan')
                    ...
                    <div class="sm:col-span-1">
                        ...
                        <select id="id_staff" name="id_staff"
                            ...
                            @foreach ($user as $k)
                                <option value="{{ $k->id }}"
                                    {{ old('id', $transaksi->id_staff) == $k->id ? 'selected' : '' }}>{{ $k->name }}
                                </option>
                            @endforeach
                        </select>
                        @error('id_staff')
                        ...
                        <select id="id_metode" name="id_metode"
                            ...
                            @foreach ($metode as $k)
                                <option value="{{ $k->id_metode }}"
                                    {{ old('id_metode', $transaksi->id_metode) == $k->id_metode ? 'selected' : '' }}>{{ $k->nama }}
                                </option>
                            @endforeach
                        </select>
                        @error('id_metode')
