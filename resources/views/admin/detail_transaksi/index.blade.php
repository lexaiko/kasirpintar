        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg">
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 p-4">
                ...
                <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                    <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                        <a href="{{ route('admin.detailTransaksi.create') }}" class="flex items-center justify-center text-white bg-yellow-400 hover:bg-yellow-400 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b dark:border-gray-700">
                            <td class="px-4 py-3">{{ ($detailTransaksis->currentPage() - 1) * $detailTransaksis->perPage() + $loop->iteration }}</td>

                            <td class="px-4 py-3">{{ $detailTransaksi->id_transaksi }}</td>
                            <td class="px-4 py-3">{{ $detailTransaksi->produk->nama }}</td>
                            ...
                            <td class="px-4 py-3 flex items-center justify-end">
                                ...
                                <!-- Dropdown menu -->
                                <div id="actionsDropdown-{{ $detailTransaksi->id_detail }}" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                    <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="actionsDropdownButton-{{ $detailTransaksi->id_detail }}">
                                        <li>
                                            <a href="{{ route('admin.detailTransaksi.edit', $detailTransaksi->id_detail) }}" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Edit</a>
                                        </li>
                                    </ul>
                                    <div class="py-1">
                                        <form action="{{ route('admin.detailTransaksi.destroy', $detailTransaksi->id_detail) }}" method="POST" onsubmit="return confirm('Apakah anda yakin ingin menghapus detail transaksi ini?');">
                                            @csrf
                                            @method('DELETE')
            ...
            <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
                <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                    ...
                    <span class="font-semibold text-gray-900 dark:text-white">
                        @if ($detailTransaksis->count())
                        {{ $detailTransaksis->firstItem() }}-{{ $detailTransaksis->lastItem() }}
                        @else
                        0-0
                        @endif
                    </span>
                    dari
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $detailTransaksis->total() }}</span>
                </span>
                <div>
                    {{ $detailTransaksis->links() }} <!-- Menampilkan link navigasi halaman -->
                </div>
            </nav>
